...
function renderDashboard(metrics) {
    ...
    output.push(`${BOLD}${CYAN}║         devpulse  dashboard      ║${RESET}`);
    output.push(`${BOLD}${CYAN}╚══════════════════════════════════╝${RESET}`);
    output.push("");
    // CPU section
    output.push(`  ${BOLD}CPU${RESET}`);
    output.push(line("Usage:", `${color}${BOLD}${metrics.cpuPercent.toString().padStart(3)}%${RESET}  ${bar(metrics.cpuPercent, 20, color)}`));
    output.push("");
    // Memory section
    const memColor = cpuColor(metrics.memPercent);
    output.push(`  ${BOLD}Memory${RESET}`);
    output.push(line("Used:", `${memColor}${BOLD}${metrics.memUsedMB} MB${RESET} / ${metrics.memTotalMB} MB  (${metrics.memPercent}%)`));
    output.push(`                ${bar(metrics.memPercent, 20, memColor)}`);
    output.push("");
    // System section
    ...
    if (metrics.topProcesses.length > 0) {
        ...
        output.push(`  ${DIM}${"PID".padEnd(8)}${"NAME".padEnd(24)}CPU%${RESET}`);
